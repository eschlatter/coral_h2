...
settlement_24_binom = data.frame(dam = settlement_24$dam[1:10],sire = settlement_24$sire[1:10],
                                 clutch = settlement_24$clutch[1:10], settled = NA)
...
for(cl in 11:nrow(settlement_24)){ #for each clutch
  if(settlement_24$lar[cl]!=0){
    for(larvs in 1:(settlement_24$lar[cl])){ #add as many rows as there are larvae in the clutch, with settled=0
      new_row <- data.frame(dam = settlement_24$dam[cl], sire = settlement_24$sire[cl], 
                            clutch = settlement_24$clutch[cl], settled = 0)
  ...
  if(settlement_24$rec[cl]!=0){
    ...
    new_row <- data.frame(dam = settlement_24$dam[cl], sire = settlement_24$sire[cl], 
                          clutch = settlement_24$clutch[cl], settled = 1)
    settlement_24_binom <- rbind(settlement_24_binom,new_row)
    }
  }
}
...
check <- summarise(group_by(settlement_24_binom,clutch),n())
check2 <- settlement_24 %>% mutate(nn = rec+lar) %>% dplyr::select(clutch,nn)

checks <-  merge(check,check2,by='clutch') %>% mutate(dif = nn-`n()`)
```
